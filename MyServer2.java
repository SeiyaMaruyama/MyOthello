...
//スレッド部（各クライアントに応じて）
class ClientProcThread2 extends Thread {
	...
	public ClientProcThread2(int n, Socket i, InputStreamReader isr, BufferedReader in, PrintWriter out) {
		number = n;
		incoming = i;
		myIsr = isr;
		myIn = in;
...
class MyServer2{
	...
	private static ClientProcThread2[] myClientProcThread;//スレッド用の配列
	...
	public static void main(String[] args) {
		...
		myClientProcThread = new ClientProcThread2[maxConnection];
		...
		try {
			...
			while (true) {
				...
				myClientProcThread[n] = new ClientProcThread2(n, incoming[n], isr[n], in[n], out[n]);//必要なパラメータを渡しスレッドを作成
